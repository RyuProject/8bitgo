...
(() => {
  ...
  // “核心还没读盘就报启动成功”的旧胶水。下面几类资源分别换代，不能为了数字整齐一起递增。
  // 桥、可执行核心和 22MB 静态资源必须独立换代。性能配置只改桥，不应逼所有玩家重新
  // 下载相同二进制；data 的 SHA-256 未变，更要复用已经铺热的 r=16 边缘/浏览器缓存。
  const RUNTIME_REVISION = '20'
  const DATA_REVISION = '17'
  const RUNTIME_SCRIPT = `PPSSPPSDL.js?r=${RUNTIME_REVISION}`
  const AUDIO_WORKLET_SCRIPT = `audio-worklet.js?r=${RUNTIME_REVISION}`
  ...
  const start = ({ requestId, file, remote, stateId }) => {
    ...
    window.Module = {
      ...
      locateFile(path) {
        ...
        url.searchParams.set('r', path.endsWith('.data') ? DATA_REVISION : RUNTIME_REVISION)
        return url.href
      },
      // Emscripten 的默认 preload fetch 一次网络抖动就永久终止启动。返回 Promise 接管同一份
      // 数据下载，在 30 秒无进度时中止当前连接并自动重试；已知长度时直接写入单一缓冲区，
      // 避免“分片数组 + 合并数组”让移动端在解包前瞬间多占一整份 data。force-cache 会优先复用 r=17。
