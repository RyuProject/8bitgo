...
import { RangeBackedFile } from "./range-backed-file.js";
...
async function handleMessage(type, payload) {
  switch (type) {
    ...
    case "mountRemote":
      return mountRemote(payload.remote);
    case "setInputMask":
...
async function mountFile(file) {
  ...
  mountedRemoteFile?.dispose();
  mountedRemoteFile = null;
  return mountFileLike(file);
}
...
async function mountRemote(remote) {
  ...
  if (!remote) {
    throw new Error("No remote disc descriptor was provided to the upstream worker");
  }
  mountedRemoteFile?.dispose();
  mountedRemoteFile = new RangeBackedFile({
    ...remote,
    onFetch: ({ requests, bytesFetched }) => {
      // 不逐扇区刷日志；每八个新块报一次，排查“是核心慢还是网络慢”时够用。
      if (requests === 1 || requests % 8 === 0) {
        postStatus(`Remote disc: ${requests} blocks / ${(bytesFetched / 1048576).toFixed(1)} MiB fetched`);
      }
    }
  });
  return mountFileLike(mountedRemoteFile);
}

async function mountFileLike(file) {
