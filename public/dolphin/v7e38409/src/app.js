...
if (EMBED_MODE) {
  document.body.classList.add("embed", "panel-hidden");
}
...
function setStatus(message, tone = "") {
  ...
  bridgePost("status", { message, tone });
}

function bridgePost(type, payload = {}, transfer = []) {
...
async function bridgeResponse(requestId, run) {
  try {
    const result = await run();
    const transfer = result?.transfer || [];
    const payload = result?.payload ?? result ?? {};
    bridgePost("response", { requestId, ok: true, payload }, transfer);
  } catch (error) {
    bridgePost("response", {
      requestId,
      ok: false,
...
if (EMBED_MODE) {
  window.addEventListener("message", (event) => {
    ...
    if (message.type === "mount-local") {
      void bridgeResponse(requestId, async () => {
        const game = await mountFile(message.file);
        if (!game || host.mode !== "dolphin") {
          throw new Error(elements.statusPill.textContent || "Dolphin rejected the selected disc");
        }
        return { game };
      });
      return;
    }
    if (message.type === "mount-remote") {
      void bridgeResponse(requestId, async () => {
        const game = await mountRemote(message.remote);
        if (!game || host.mode !== "dolphin") {
          throw new Error(elements.statusPill.textContent || "Dolphin rejected the remote disc");
        }
        return { game };
      });
      return;
    ...
    if (message.type === "set-volume") {
      ...
      void audio.setMuted(volume === 0).then(() => host.setAudioMuted(audio.muted));
      return;
    }
    ...
    if (message.type === "save-state") {
      void bridgeResponse(requestId, async () => {
        ...
        if (!adapter || typeof adapter.saveStateFile !== "function") {
          throw new Error("Dolphin save states are not available");
        }
        const saved = await adapter.saveStateFile();
        if (!saved?.bytes) throw new Error(saved?.error || "Dolphin returned an empty save state");
        const view = new Uint8Array(saved.bytes);
        const bytes = view.buffer.slice(view.byteOffset, view.byteOffset + view.byteLength);
        return { payload: { bytes }, transfer: [bytes] };
      });
      return;
    }
    if (message.type === "load-state") {
      void bridgeResponse(requestId, async () => {
        const adapter = host.adapter;
        if (!adapter || typeof adapter.loadStateFile !== "function") {
          throw new Error("Dolphin save states are not available");
        }
        const loaded = await adapter.loadStateFile(new Uint8Array(message.bytes));
        if (!loaded?.loaded) throw new Error(loaded?.error || "Dolphin rejected the save state");
        return loaded;
      });
    }
  });
...
async function mountFile(file) {
  try {
    ...
    await activateMountedGame(game);
    return game;
  } catch (error) {
    setStatus(error.message, "error");
    return null;
...
async function mountRemote(remote) {
  try {
    const game = await host.mountRemote(remote);
    await activateMountedGame(game);
    return game;
  } catch (error) {
    setStatus(error.message, "error");
...
async function activateMountedGame(game) {
  // 本地选盘本身就是用户手势，可以直接开声音；独立播放器自动启动时浏览器可能拦截，
  // 那种情况保持静音，等玩家点音量按钮后再恢复，不让自动播放策略阻断整个启动。
  if (audio.muted) {
    try {
