...
export class EmulatorHost {
  ...
  async mountFile(file) {
    return this.mountSource(file, () => this.adapter.mountGame(file));
  }

  async mountRemote(remote) {
    if (typeof this.adapter.mountRemote !== "function") {
      throw new Error("The selected Dolphin adapter cannot stream a remote disc");
    }
    return this.mountSource(remote, () => this.adapter.mountRemote(remote));
  }

  async mountSource(source, mount) {
    this.game = {
      name: source.name,
      size: source.size,
      mounted: true
    ...
    if (await this.adapterAvailable()) {
      try {
        ...
        this.mode = "dolphin";
        this.game.name = mounted.title || source.name;
