...
export class UpstreamWorkerAdapter {
  ...
  async mountRemote(remote) {
    const _t_mountStart = performance.now();
    await this.load();
    const response = await this.request("mountRemote", { remote });
    console.log(
      `[boot-phase] mountRemote() completed in ${(performance.now() - _t_mountStart).toFixed(1)}ms ` +
        `(size=${remote?.size ?? "?"}B name=${remote?.name ?? "?"})`
    );
    this.applyMetadata(response);
    this.applyFrame(response);
