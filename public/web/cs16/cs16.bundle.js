// public/web/cs16/engine/dist/constants.js
...
// public/web/cs16/engine/dist/generated/xash.js
var Xash3D = (() => {
  ...
  return async function(moduleArg = {}) {
    ...
    function getWasmImports() {
      var GOTProxyHandler = new Proxy(/* @__PURE__ */ new Set(["__start___llvm_prf_data", "__stop___llvm_prf_data", "__start___llvm_prf_names", "__stop___llvm_prf_names", "__start___llvm_prf_vns", "__stop___llvm_prf_vns", "__start___llvm_prf_vtab", "__stop___llvm_prf_vtab", "__start___llvm_prf_cnts", "__stop___llvm_prf_cnts", "__start___llvm_prf_bits", "__stop___llvm_prf_bits", "__start___llvm_prf_vnds", "__stop___llvm_prf_vnds"]), GOTHandler);
      var imports = { env: wasmImports, wasi_snapshot_preview1: wasmImports, "GOT.mem": GOTProxyHandler, "GOT.func": GOTProxyHandler };
      return imports;
    }
    ...
    var convertJsFunctionToWasm = (func2, sig2) => {
      var bytes = Uint8Array.of(0, 97, 115, 109, 1, 0, 0, 0, 1, ...uleb128EncodeWithLen([1, 96, ...generateTypePack(sig2.slice(1)), ...generateTypePack(sig2[0] === "v" ? "" : sig2[0])]), 2, 7, 1, 1, 101, 1, 102, 0, 0, 7, 5, 1, 1, 102, 0, 0);
      var module2 = new WebAssembly.Module(bytes);
      var instance2 = new WebAssembly.Instance(module2, { e: { f: func2 } });
      var wrappedFunc = instance2.exports["f"];
      return wrappedFunc;
    };
    var addFunction = (func2, sig2) => {
      ...
      try {
        ...
        var wrapped = convertJsFunctionToWasm(func2, sig2);
    ...
    var relocateExports = (exports, memoryBase2 = 0) => {
      function relocateExport(name2, value) {
        if (isImmutableGlobal(value)) {
          return new WebAssembly.Global({ value: "i32" }, value.value + memoryBase2);
        }
        return value;
      }
      var relocated = {};
    ...
    var createNamedFunction = (name2, func2) => Object.defineProperty(func2, "name", { value: name2 });
    var dynCall = (sig2, ptr2, args2 = [], promising = false) => {
      var func2 = getWasmTableEntry(ptr2);
      var rtn = func2(...args2);
    ...
    var createInvokeFunction = (sig2) => (ptr2, ...args2) => {
      var sp = stackSave();
      try {
        return dynCall(sig2, ptr2, args2);
      } catch (e) {
        stackRestore(sp);
        if (e !== e + 0)
          throw e;
    ...
    var UTF8ToString = (ptr2, maxBytesToRead, ignoreNul) => {
      if (!ptr2)
        return "";
      var end = findStringEnd(HEAPU8, ptr2, maxBytesToRead, ignoreNul);
      return UTF8Decoder.decode(HEAPU8.subarray(ptr2, end));
    };
    var loadWebAssemblyModule = (binary, flags, libName, localScope, handle) => {
      ...
      function loadModule() {
        ...
        var info = { "GOT.mem": GOTProxy, "GOT.func": GOTProxy, env: proxy, wasi_snapshot_preview1: proxy };
      ...
      if (flags.loadAsync) {
        return metadata.neededDynlibs.reduce((chain, needed2) => chain.then(() => {
          needed2 = findLibraryFS(needed2, flags.rpath) ?? needed2;
          return loadDynamicLibrary(needed2, flags, localScope);
    ...
    var registerWasmPlugin = () => {
      var wasmPlugin = { promiseChainEnd: Promise.resolve(), canHandle: (name2) => !Module["noWasmDecoding"] && name2.endsWith(".so"), handle: async (byteArray, name2) => wasmPlugin.promiseChainEnd = wasmPlugin.promiseChainEnd.then(async () => {
        try {
          var exports = await loadWebAssemblyModule(byteArray, { loadAsync: true, nodelete: true }, name2, {});
        } catch (error) {
          throw new Error(`failed to instantiate wasm: ${name2}: ${error}`);
        }
    ...
    var PATH_FS = { resolve: (...args2) => {
      ...
      function trim(arr) {
        ...
        if (start3 > end)
          return [];
        return arr.slice(start3, end - start3 + 1);
      }
    ...
    var IDBFS = { dbs: {}, indexedDB: () => indexedDB, DB_VERSION: 21, DB_STORE_NAME: "FILE_DATA", queuePersist: (mount) => {
      ...
      var total = 0;
      var create = [];
      for (var [key2, e] of Object.entries(src.entries)) {
      ...
      var remove = [];
      for (var key2 of Object.keys(dst.entries)) {
        if (!src.entries[key2]) {
          remove.push(key2);
          total++;
        }
      ...
      for (const path2 of create.sort()) {
        if (dst.type === "local") {
          IDBFS.loadRemoteEntry(store, path2, (err3, entry) => {
            if (err3)
              return done(err3);
            IDBFS.storeLocalEntry(path2, entry, done);
          });
        } else {
          IDBFS.loadLocalEntry(path2, (err3, entry) => {
            if (err3)
              return done(err3);
    ...
    var FS_preloadFile = async (parent, name2, url, canRead, canWrite, dontCreateFile, canOwn, preFinish) => {
      ...
      try {
        ...
        preFinish?.();
        if (!dontCreateFile) {
          FS_createDataFile(parent, name2, byteArray, canRead, canWrite, canOwn);
        }
      } finally {
    ...
    var FS_createPreloadedFile = (parent, name2, url, canRead, canWrite, onload, onerror, dontCreateFile, canOwn, preFinish) => {
      FS_preloadFile(parent, name2, url, canRead, canWrite, dontCreateFile, canOwn, preFinish).then(onload).catch(onerror);
    };
    var FS = { root: null, mounts: [], devices: {}, streams: [], nextInode: 1, nameTable: null, currentPath: "/", initialized: false, ignorePermissions: true, filesystems: null, syncFSRequests: 0, readFiles: {}, ErrnoError: class {
    ...
    }, checkOpExists(op, err3) {
      if (!op) {
        throw new FS.ErrnoError(err3);
      }
      return op;
        ...
        cacheLength() {
          ...
          lazyArray2.setDataGetter((chunkNum) => {
            ...
            if (typeof lazyArray2.chunks[chunkNum] == "undefined") {
              lazyArray2.chunks[chunkNum] = doXHR(start3, end);
            }
            if (typeof lazyArray2.chunks[chunkNum] == "undefined")
              abort("doXHR failed!");
      ...
      var stream_ops = {};
      for (const [key2, fn] of Object.entries(node.stream_ops)) {
        stream_ops[key2] = (...args2) => {
    ...
    var ___call_sighandler = (fp, sig2) => getWasmTableEntry(fp)(sig2);
    ...
    function ___syscall_fchmodat2(dirfd, path, mode, flags2) {
      try {
    ...
    function ___syscall_recvmsg(fd, message, flags2, d1, d2, d3) {
      try {
        ...
        for (var i2 = 0; i2 < num; i2++) {
          total += HEAP32[iov + (8 * i2 + 4) >> 2];
        }
        var msg = sock.sock_ops.recvmsg(sock, total);
        if (!msg)
          return 0;
        var name2 = HEAPU32[message >> 2];
        if (name2) {
        ...
        for (var i2 = 0; bytesRemaining > 0 && i2 < num; i2++) {
          var iovbase = HEAPU32[iov + (8 * i2 + 0) >> 2];
          var iovlen = HEAP32[iov + (8 * i2 + 4) >> 2];
          if (!iovlen) {
            continue;
          }
    ...
    function ___syscall_sendmsg(fd, message, flags2, d1, d2, d3) {
      try {
        ...
        if (name2) {
          var info2 = getSocketAddress(name2, namelen);
          port = info2.port;
          addr2 = info2.addr;
        }
        var total = 0;
...
${e}`);
    ...
    var __dlopen_js = (handle2) => dlopenInternal(handle2, { loadAsync: false });
    __dlopen_js.sig = "pp";
    ...
    var __emscripten_dlopen_js = (handle2, onsuccess, onerror, user_data) => {
      function errorCallback(e) {
        ...
        dlSetError(`'Could not load dynamic lib: ${filename}
${e}`);
        callUserCallback(() => getWasmTableEntry(onerror)(handle2, user_data));
      }
    ...
    var JSEvents = { removeAllEventListeners() {
      ...
      let success = false;
      for (let i2 = 0; i2 < JSEvents.eventHandlers.length; ++i2) {
        const handler = JSEvents.eventHandlers[i2];
...
`;
    ...
    function __msync_js(addr2, len, prot, flags2, fd, offset) {
      offset = bigintToI53Checked(offset);
      try {
        if (isNaN(offset))
          return -61;
        SYSCALLS.doMsync(addr2, SYSCALLS.getStreamFromFD(fd), len, flags2, offset);
        return 0;
      } catch (e) {
        if (typeof FS == "undefined" || !(e.name === "ErrnoError"))
          throw e;
    ...
    function __poll_js(fds, nfds, timeout, ctx, arg2) {
      try {
        var count = 0;
        for (var i2 = 0; i2 < nfds; i2++) {
          ...
          if (stream) {
            if (stream.stream_ops.poll) {
              flags2 = stream.stream_ops.poll(stream, -1);
            } else {
              flags2 = 5;
            }
          }
    ...
    var setMainLoop = (iterFunc, fps, simulateInfiniteLoop, arg2, noSetTiming) => {
    ...
    var MainLoop = { running: false, scheduler: null, method: "", currentlyRunningMainloop: 0, func: null, arg: 0, timingMode: 0, timingValue: 0, currentFrameNumber: 0, queue: [], preMainLoop: [], postMainLoop: [], pause() {
      ...
      if (globalThis.requestAnimationFrame) {
        ...
        MainLoop.fakeRequestAnimationFrame(func2);
      }
    } };
    var AL = { QUEUE_INTERVAL: 25, QUEUE_LOOKAHEAD: 0.1, DEVICE_NAME: "Emscripten OpenAL", CAPTURE_DEVICE_NAME: "Emscripten OpenAL capture", ALC_EXTENSIONS: { ALC_EXT_capture: true, ALC_SOFT_pause_device: true, ALC_SOFT_HRTF: true }, AL_EXTENSIONS: { AL_EXT_float32: true, AL_SOFT_loop_points: true, AL_SOFT_source_length: true, AL_EXT_source_distance_model: true, AL_SOFT_source_spatialize: true }, _alcErr: 0, alcErr: 0, deviceRefCounts: {}, alcStringCache: {}, paused: false, stringCache: {}, contexts: {}, currentCtx: null, buffers: { 0: { id: 0, refCount: 0, audioBuf: null, frequency: 0, bytesPerSample: 2, channels: 1, length: 0 } }, paramArray: [], _nextId: 1, newId: () => AL.freeIds.length > 0 ? AL.freeIds.pop() : AL._nextId++, freeIds: [], scheduleContextAudio: (ctx) => {
    ...
    var Browser = { useWebGL: false, isFullscreen: false, pointerLock: false, moduleContextCreatedCallbacks: [], workers: [], preloadedImages: {}, preloadedAudios: {}, getCanvas: () => Module["canvas"], init() {
      ...
      imagePlugin["handle"] = async function imagePlugin_handle(byteArray, name2) {
      ...
      audioPlugin["handle"] = async function audioPlugin_handle(byteArray, name2) {
        return new Promise((resolve, reject) => {
          var done = false;
          ...
          audio.onerror = function audio_onerror(event) {
            ...
            function encode64(data) {
              var BASE2 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
              var PAD = "=";
              var ret = "";
              var leftchar = 0;
              var leftbits = 0;
              ...
              if (leftbits == 2) {
                ret += BASE2[(leftchar & 3) << 4];
                ret += PAD + PAD;
              } else if (leftbits == 4) {
                ret += BASE2[(leftchar & 15) << 2];
                ret += PAD;
              }
              return ret;
            }
    ...
    var GL = { counter: 1, buffers: [], programs: [], framebuffers: [], renderbuffers: [], textures: [], shaders: [], vaos: [], contexts: [], offscreenCanvases: {}, queries: [], samplers: [], transformFeedbacks: [], syncs: [], byteSizeByTypeRoot: 5120, byteSizeByType: [1, 1, 2, 2, 4, 4, 4, 2, 3, 4, 8], stringCache: {}, stringiCache: {}, unpackAlignment: 4, unpackRowLength: 0, recordError: (errorCode) => {
      if (!GL.lastError) {
    ...
    }, MAX_TEMP_BUFFER_SIZE: 2097152, numTempVertexBuffersPerSize: 64, log2ceilLookup: (i2) => 32 - Math.clz32(i2 === 0 ? 0 : i2 - 1), generateTempBuffers: (quads, context) => {
    ...
    }, usedTempBuffers: [], preDrawHandleClientVertexAttribBindings: (count) => {
    ...
    var _emscripten_glBindAttribLocation = (program, index, name2) => {
    ...
    var _emscripten_glClientWaitSync = (sync, flags2, timeout) => {
      timeout = Number(timeout);
      return GLctx.clientWaitSync(GL.syncs[sync], flags2, timeout);
    };
    ...
    var _emscripten_glDrawRangeElements = (mode, start3, end, count, type, indices) => {
    ...
    var _emscripten_glFenceSync = (condition, flags2) => {
      var sync = GLctx.fenceSync(condition, flags2);
      if (sync) {
    ...
    var __glGetActiveAttribOrUniform = (funcName, program, index, bufSize, length, size, type, name2) => {
      ...
      if (info2) {
        ...
        if (size)
          HEAP32[size >> 2] = info2.size;
        if (type)
          HEAP32[type >> 2] = info2.type;
      }
    };
    var _emscripten_glGetActiveAttrib = (program, index, bufSize, length, size, type, name2) => __glGetActiveAttribOrUniform("getActiveAttrib", program, index, bufSize, length, size, type, name2);
    _emscripten_glGetActiveAttrib.sig = "viiipppp";
    var _emscripten_glGetActiveUniform = (program, index, bufSize, length, size, type, name2) => __glGetActiveAttribOrUniform("getActiveUniform", program, index, bufSize, length, size, type, name2);
    ...
    var _emscripten_glGetActiveUniformBlockiv = (program, uniformBlockIndex, pname, params) => {
      ...
      program = GL.programs[program];
      if (pname == 35393) {
        var name2 = GLctx.getActiveUniformBlockName(program, uniformBlockIndex);
        HEAP32[params >> 2] = name2.length + 1;
        return;
      }
    ...
    var _emscripten_glGetAttribLocation = (program, name2) => GLctx.getAttribLocation(GL.programs[program], UTF8ToString(name2));
    ...
    var _emscripten_glGetFragDataLocation = (program, name2) => GLctx.getFragDataLocation(GL.programs[program], UTF8ToString(name2));
    ...
    var _emscripten_glGetProgramBinary = (program, bufSize, length, binaryFormat, binary2) => {
      GL.recordError(1282);
    };
    _emscripten_glGetProgramBinary.sig = "viippp";
    var _emscripten_glGetProgramInfoLog = (program, maxLength, length, infoLog) => {
      var log2 = GLctx.getProgramInfoLog(GL.programs[program]);
      if (log2 === null)
        log2 = "(unknown error)";
      var numBytesWrittenExclNull = maxLength > 0 && infoLog ? stringToUTF8(log2, infoLog, maxLength) : 0;
      if (length)
        HEAP32[length >> 2] = numBytesWrittenExclNull;
    };
    ...
    var _emscripten_glGetProgramiv = (program, pname, p) => {
      ...
      program = GL.programs[program];
      if (pname == 35716) {
        var log2 = GLctx.getProgramInfoLog(program);
        if (log2 === null)
          log2 = "(unknown error)";
        HEAP32[p >> 2] = log2.length + 1;
      } else if (pname == 35719) {
    ...
    var _emscripten_glGetShaderInfoLog = (shader, maxLength, length, infoLog) => {
      var log2 = GLctx.getShaderInfoLog(GL.shaders[shader]);
      if (log2 === null)
        log2 = "(unknown error)";
      var numBytesWrittenExclNull = maxLength > 0 && infoLog ? stringToUTF8(log2, infoLog, maxLength) : 0;
      if (length)
    ...
    var _emscripten_glGetShaderiv = (shader, pname, p) => {
      ...
      if (pname == 35716) {
        ...
        if (log2 === null)
          log2 = "(unknown error)";
        var logLength = log2 ? log2.length + 1 : 0;
    ...
    var _emscripten_glGetStringi = (name2, index) => {
      if (GL.currentContext.version < 2) {
        GL.recordError(1282);
        return 0;
      }
      var stringiCache = GL.stringiCache[name2];
      if (stringiCache) {
      ...
      switch (name2) {
        case 7939:
          ...
          stringiCache = GL.stringiCache[name2] = exts;
          if (index < 0 || index >= stringiCache.length) {
            GL.recordError(1281);
            return 0;
          }
    ...
    var _emscripten_glGetTransformFeedbackVarying = (program, index, bufSize, length, size, type, name2) => {
      program = GL.programs[program];
      var info2 = GLctx.getTransformFeedbackVarying(program, index);
      if (!info2)
        return;
      if (name2 && bufSize > 0) {
        var numBytesWrittenExclNull = stringToUTF8(info2.name, name2, bufSize);
        if (length)
          HEAP32[length >> 2] = numBytesWrittenExclNull;
      } else {
        if (length)
    ...
    var _emscripten_glGetUniformIndices = (program, uniformCount, uniformNames, uniformIndices) => {
      ...
      var names = [];
      for (var i2 = 0; i2 < uniformCount; i2++)
        names.push(UTF8ToString(HEAPU32[uniformNames + i2 * 4 >> 2]));
    ...
    var _emscripten_glGetUniformLocation = (program, name2) => {
    ...
    var emscriptenWebGLGetUniform = (program, location2, params, type) => {
      if (!params) {
        GL.recordError(1281);
        return;
      }
    ...
    var _emscripten_glGetUniformfv = (program, location2, params) => {
      emscriptenWebGLGetUniform(program, location2, params, 2);
    };
    _emscripten_glGetUniformfv.sig = "viip";
    var _emscripten_glGetUniformiv = (program, location2, params) => {
      emscriptenWebGLGetUniform(program, location2, params, 0);
    };
    _emscripten_glGetUniformiv.sig = "viip";
    var _emscripten_glGetUniformuiv = (program, location2, params) => emscriptenWebGLGetUniform(program, location2, params, 0);
    ...
    var _emscripten_glProgramBinary = (program, binaryFormat, binary2, length) => {
      GL.recordError(1280);
    };
    ...
    var _emscripten_glShaderBinary = (count, shaders, binaryformat, binary2, length) => {
      GL.recordError(1280);
    };
    ...
    var _emscripten_glUniformMatrix2fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix2x3fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix2x4fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix3fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix3x2fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix3x4fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix4fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix4x2fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glUniformMatrix4x3fv = (location2, count, transpose, value) => {
    ...
    var _emscripten_glVertexAttribIPointer = (index, size, type, stride, ptr2) => {
      GLctx.vertexAttribIPointer(index, size, type, stride, ptr2);
    };
    _emscripten_glVertexAttribIPointer.sig = "viiiip";
    var _emscripten_glVertexAttribPointer = (index, size, type, normalized, stride, ptr2) => {
      var cb = GL.currentContext.clientBuffers[index];
      if (!GLctx.currentArrayBufferBinding) {
        ...
        cb.ptr = ptr2;
        cb.clientside = true;
        cb.vertexAttribPointerAdaptor = function(index2, size2, type2, normalized2, stride2, ptr3) {
          this.vertexAttribPointer(index2, size2, type2, normalized2, stride2, ptr3);
        };
        return;
      }
      cb.clientside = false;
    ...
    var _emscripten_glWaitSync = (sync, flags2, timeout) => {
      timeout = Number(timeout);
      GLctx.waitSync(GL.syncs[sync], flags2, timeout);
    };
    ...
    var _emscripten_net_gethostname = (name2, namelen) => Module.net?.gethostname?.(name2, namelen) ?? -1;
    var _emscripten_net_getsockname = (fd, sockaddr, socklen) => Module.net?.getsockname?.(fd, sockaddr, socklen) ?? -1;
    var _emscripten_net_recvfrom = (fd, buf, len, flags2, sockaddr, socklen) => Module.net?.recvfrom?.(fd, buf, len, flags2, sockaddr, socklen) ?? -1;
    var _emscripten_net_sendto = (fd, buf, len, flags2, sockaddr, socklen) => Module.net?.sendto?.(fd, buf, len, flags2, sockaddr, socklen) ?? -1;
    var _emscripten_net_sendto_batch = (fd, bufs, lens, flags2, sockaddr, socklen) => Module.net?.sendto_batch?.(fd, bufs, lens, flags2, sockaddr, socklen) ?? -1;
    ...
    var _emscripten_resize_heap = (requestedSize) => {
      ...
      if (requestedSize > 536870912) return 0;
      var target = Math.min(536870912, Math.max(requestedSize, Math.ceil(oldSize * 1.2 / 65536) * 65536));
      return growMemory(target);
    };
    ...
    var registerBeforeUnloadEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString) => {
      ...
      var eventHandler = { target: findEventTarget(target), eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: beforeUnloadEventHandlerFunc, useCapture };
    ...
    var registerKeyEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      var eventSize = 160;
      JSEvents.keyEvent || (JSEvents.keyEvent = _malloc(eventSize));
      ...
      var eventHandler = { target: findEventTarget(target), eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: keyEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var _emscripten_set_main_loop_arg = (func2, arg2, fps, simulateInfiniteLoop) => {
      var iterFunc = () => getWasmTableEntry(func2)(arg2);
      setMainLoop(iterFunc, fps, simulateInfiniteLoop, arg2);
    };
    ...
    var registerMouseEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      ...
      var eventHandler = { target, allowsDeferredCalls: eventTypeString != "mousemove" && eventTypeString != "mouseenter" && eventTypeString != "mouseleave", eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: mouseEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var registerTouchEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      ...
      var eventHandler = { target, allowsDeferredCalls: eventTypeString == "touchstart" || eventTypeString == "touchend", eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: touchEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var registerWheelEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      var eventSize = 96;
      JSEvents.wheelEvent || (JSEvents.wheelEvent = _malloc(eventSize));
      ...
      var eventHandler = { target, allowsDeferredCalls: true, eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: wheelHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var writeI53ToI64Clamped = (ptr2, num) => {
      if (num > 9223372036854776e3) {
        HEAPU32[ptr2 >> 2] = 4294967295;
        HEAPU32[ptr2 + 4 >> 2] = 2147483647;
      } else if (num < -9223372036854776e3) {
        HEAPU32[ptr2 >> 2] = 0;
        HEAPU32[ptr2 + 4 >> 2] = 2147483648;
      } else {
    ...
    var _emscripten_get_compiler_setting = (name2) => abort("You must build with -sRETAIN_COMPILER_SETTINGS for getCompilerSetting or emscripten_get_compiler_setting to work");
    ...
    var __Unwind_Backtrace = (func2, arg2) => {
      var trace = getCallstack();
      var parts = trace.split("\n");
      for (var i2 = 0; i2 < parts.length; i2++) {
        var ret = getWasmTableEntry(func2)(0, arg2);
        if (ret !== 0)
          return;
      }
    };
    ...
    var getDynCaller = (sig2, ptr2, promising = false) => (...args2) => dynCall(sig2, ptr2, args2, promising);
    ...
    var UTF16ToString = (ptr2, maxBytesToRead, ignoreNul) => {
      var idx = ptr2 >> 1;
    ...
    var UTF32ToString = (ptr2, maxBytesToRead, ignoreNul) => {
      var str = "";
      var startIdx = ptr2 >> 2;
    ...
    var _emscripten_html5_remove_event_listener = (target, userData, eventTypeId, callback) => {
      var eventHandler = { target: findEventTarget(target), userData, eventTypeId, callbackfunc: callback };
    ...
    var registerDeviceOrientationEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      var eventSize = 32;
      JSEvents.deviceOrientationEvent || (JSEvents.deviceOrientationEvent = _malloc(eventSize));
      ...
      var eventHandler = { target: findEventTarget(target), eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: deviceOrientationEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var registerDeviceMotionEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      var eventSize = 80;
      JSEvents.deviceMotionEvent || (JSEvents.deviceMotionEvent = _malloc(eventSize));
      ...
      var eventHandler = { target: findEventTarget(target), eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: deviceMotionEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var registerOrientationChangeEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      var eventSize = 8;
      JSEvents.orientationChangeEvent || (JSEvents.orientationChangeEvent = _malloc(eventSize));
      ...
      var eventHandler = { target, eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: orientationChangeEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var registerPointerlockErrorEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      ...
      var eventHandler = { target, eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: pointerlockErrorEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var registerBatteryEventCallback = (battery, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      var eventSize = 32;
      JSEvents.batteryEvent || (JSEvents.batteryEvent = _malloc(eventSize));
      ...
      var eventHandler = { target: battery, eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: batteryEventHandlerFunc, useCapture };
      return JSEvents.registerOrRemoveHandler(eventHandler);
    };
    ...
    var _emscripten_async_call = (func2, arg2, millis) => {
      var wrapper = () => getWasmTableEntry(func2)(arg2);
      if (millis >= 0) {
    ...
    var __emscripten_push_main_loop_blocker = (func2, arg2, name2) => {
      MainLoop.queue.push({ func: () => {
        getWasmTableEntry(func2)(arg2);
      }, name: UTF8ToString(name2), counted: true });
      MainLoop.updateStatus();
    };
    __emscripten_push_main_loop_blocker.sig = "vppp";
    var __emscripten_push_uncounted_main_loop_blocker = (func2, arg2, name2) => {
      MainLoop.queue.push({ func: () => {
        getWasmTableEntry(func2)(arg2);
    ...
    var _emscripten_promise_any = (idBuf, errorBuf, size) => {
      var promises = idsToPromises(idBuf, size);
      var id = promiseMap.allocate({ promise: Promise.any(promises).catch((err3) => {
        if (errorBuf) {
          for (var i2 = 0; i2 < size; i2++) {
            HEAPU32[errorBuf + i2 * 4 >> 2] = err3.errors[i2];
          }
        }
        throw errorBuf;
      }) });
    ...
    var _emscripten_run_preload_plugins_data = (data, size, suffix, arg2, onload, onerror) => {
      ...
      FS.createPreloadedFile("/", name2, HEAPU8.subarray(data, data + size), true, true, () => {
        if (onload)
          getWasmTableEntry(onload)(arg2, cname);
      }, () => {
        if (onerror)
          getWasmTableEntry(onerror)(arg2);
      }, true);
    };
    ...
    var _emscripten_call_worker = (id, funcName, data, size, callback, arg2) => {
      funcName = UTF8ToString(funcName);
      var info2 = Browser.workers[id];
      var callbackId = -1;
      if (callback) {
        callbackId = info2.callbacks.length;
        info2.callbacks.push({ func: getWasmTableEntry(callback), arg: arg2 });
        info2.awaited++;
      }
    ...
    var FS_unlink = (...args2) => FS.unlink(...args2);
    ...
    var _emscripten_async_wget2 = (url, file, request, param, userdata, onload, onerror, onprogress) => {
      ...
      http.onload = (e) => {
        if (http.status >= 200 && http.status < 300) {
          ...
          if (onload) {
            var sp = stackSave();
            getWasmTableEntry(onload)(handle2, userdata, stringToUTF8OnStack(_file));
            stackRestore(sp);
          }
        } else {
          if (onerror)
            getWasmTableEntry(onerror)(handle2, userdata, http.status);
        }
    ...
    var _emscripten_async_wget2_data = (url, request, param, userdata, free, onload, onerror, onprogress) => {
      ...
      function onerrorjs() {
        if (onerror) {
          ...
          getWasmTableEntry(onerror)(handle2, userdata, http.status, statusText);
          stackRestore(sp);
        }
      }
      http.onload = (e) => {
        if (http.status >= 200 && http.status < 300 || http.status === 0 && _url.slice(0, 4).toLowerCase() != "http") {
          ...
          HEAPU8.set(byteArray, buffer);
          if (onload)
            getWasmTableEntry(onload)(handle2, userdata, buffer, byteArray.length);
          if (free)
            _free(buffer);
        } else {
          onerrorjs();
    ...
    var FS_createPath = (...args2) => FS.createPath(...args2);
    var FS_createDevice = (...args2) => FS.createDevice(...args2);
    var FS_readFile = (...args2) => FS.readFile(...args2);
    var FS_root = (...args2) => FS.root(...args2);
    var FS_mounts = (...args2) => FS.mounts(...args2);
    var FS_devices = (...args2) => FS.devices(...args2);
    var FS_streams = (...args2) => FS.streams(...args2);
    var FS_nextInode = (...args2) => FS.nextInode(...args2);
    var FS_nameTable = (...args2) => FS.nameTable(...args2);
    var FS_currentPath = (...args2) => FS.currentPath(...args2);
    var FS_initialized = (...args2) => FS.initialized(...args2);
    var FS_ignorePermissions = (...args2) => FS.ignorePermissions(...args2);
    var FS_filesystems = (...args2) => FS.filesystems(...args2);
    var FS_syncFSRequests = (...args2) => FS.syncFSRequests(...args2);
    var FS_readFiles = (...args2) => FS.readFiles(...args2);
    var FS_lookupPath = (...args2) => FS.lookupPath(...args2);
    var FS_getPath = (...args2) => FS.getPath(...args2);
    var FS_hashName = (...args2) => FS.hashName(...args2);
    var FS_hashAddNode = (...args2) => FS.hashAddNode(...args2);
    var FS_hashRemoveNode = (...args2) => FS.hashRemoveNode(...args2);
    var FS_lookupNode = (...args2) => FS.lookupNode(...args2);
    var FS_createNode = (...args2) => FS.createNode(...args2);
    var FS_destroyNode = (...args2) => FS.destroyNode(...args2);
    var FS_isRoot = (...args2) => FS.isRoot(...args2);
    var FS_isMountpoint = (...args2) => FS.isMountpoint(...args2);
    var FS_isFile = (...args2) => FS.isFile(...args2);
    var FS_isDir = (...args2) => FS.isDir(...args2);
    var FS_isLink = (...args2) => FS.isLink(...args2);
    var FS_isChrdev = (...args2) => FS.isChrdev(...args2);
    var FS_isBlkdev = (...args2) => FS.isBlkdev(...args2);
    var FS_isFIFO = (...args2) => FS.isFIFO(...args2);
    var FS_isSocket = (...args2) => FS.isSocket(...args2);
    var FS_flagsToPermissionString = (...args2) => FS.flagsToPermissionString(...args2);
    var FS_nodePermissions = (...args2) => FS.nodePermissions(...args2);
    var FS_mayLookup = (...args2) => FS.mayLookup(...args2);
    var FS_mayCreate = (...args2) => FS.mayCreate(...args2);
    var FS_mayDelete = (...args2) => FS.mayDelete(...args2);
    var FS_mayOpen = (...args2) => FS.mayOpen(...args2);
    var FS_checkOpExists = (...args2) => FS.checkOpExists(...args2);
    var FS_nextfd = (...args2) => FS.nextfd(...args2);
    var FS_getStreamChecked = (...args2) => FS.getStreamChecked(...args2);
    var FS_getStream = (...args2) => FS.getStream(...args2);
    var FS_createStream = (...args2) => FS.createStream(...args2);
    var FS_closeStream = (...args2) => FS.closeStream(...args2);
    var FS_dupStream = (...args2) => FS.dupStream(...args2);
    var FS_doSetAttr = (...args2) => FS.doSetAttr(...args2);
    var FS_chrdev_stream_ops = (...args2) => FS.chrdev_stream_ops(...args2);
    var FS_major = (...args2) => FS.major(...args2);
    var FS_minor = (...args2) => FS.minor(...args2);
    var FS_makedev = (...args2) => FS.makedev(...args2);
    var FS_registerDevice = (...args2) => FS.registerDevice(...args2);
    var FS_getDevice = (...args2) => FS.getDevice(...args2);
    var FS_getMounts = (...args2) => FS.getMounts(...args2);
    var FS_syncfs = (...args2) => FS.syncfs(...args2);
    var FS_mount = (...args2) => FS.mount(...args2);
    var FS_unmount = (...args2) => FS.unmount(...args2);
    var FS_lookup = (...args2) => FS.lookup(...args2);
    var FS_mknod = (...args2) => FS.mknod(...args2);
    var FS_statfs = (...args2) => FS.statfs(...args2);
    var FS_statfsStream = (...args2) => FS.statfsStream(...args2);
    var FS_statfsNode = (...args2) => FS.statfsNode(...args2);
    var FS_create = (...args2) => FS.create(...args2);
    var FS_mkdir = (...args2) => FS.mkdir(...args2);
    var FS_mkdev = (...args2) => FS.mkdev(...args2);
    var FS_symlink = (...args2) => FS.symlink(...args2);
    var FS_rename = (...args2) => FS.rename(...args2);
    var FS_rmdir = (...args2) => FS.rmdir(...args2);
    var FS_readdir = (...args2) => FS.readdir(...args2);
    var FS_readlink = (...args2) => FS.readlink(...args2);
    var FS_stat = (...args2) => FS.stat(...args2);
    var FS_fstat = (...args2) => FS.fstat(...args2);
    var FS_lstat = (...args2) => FS.lstat(...args2);
    var FS_doChmod = (...args2) => FS.doChmod(...args2);
    var FS_chmod = (...args2) => FS.chmod(...args2);
    var FS_lchmod = (...args2) => FS.lchmod(...args2);
    var FS_fchmod = (...args2) => FS.fchmod(...args2);
    var FS_doChown = (...args2) => FS.doChown(...args2);
    var FS_chown = (...args2) => FS.chown(...args2);
    var FS_lchown = (...args2) => FS.lchown(...args2);
    var FS_fchown = (...args2) => FS.fchown(...args2);
    var FS_doTruncate = (...args2) => FS.doTruncate(...args2);
    var FS_truncate = (...args2) => FS.truncate(...args2);
    var FS_ftruncate = (...args2) => FS.ftruncate(...args2);
    var FS_utime = (...args2) => FS.utime(...args2);
    var FS_open = (...args2) => FS.open(...args2);
    var FS_close = (...args2) => FS.close(...args2);
    var FS_isClosed = (...args2) => FS.isClosed(...args2);
    var FS_llseek = (...args2) => FS.llseek(...args2);
    var FS_read = (...args2) => FS.read(...args2);
    var FS_write = (...args2) => FS.write(...args2);
    var FS_mmap = (...args2) => FS.mmap(...args2);
    var FS_msync = (...args2) => FS.msync(...args2);
    var FS_ioctl = (...args2) => FS.ioctl(...args2);
    var FS_writeFile = (...args2) => FS.writeFile(...args2);
    var FS_cwd = (...args2) => FS.cwd(...args2);
    var FS_chdir = (...args2) => FS.chdir(...args2);
    var FS_createDefaultDirectories = (...args2) => FS.createDefaultDirectories(...args2);
    var FS_createDefaultDevices = (...args2) => FS.createDefaultDevices(...args2);
    var FS_createSpecialDirectories = (...args2) => FS.createSpecialDirectories(...args2);
    var FS_createStandardStreams = (...args2) => FS.createStandardStreams(...args2);
    var FS_staticInit = (...args2) => FS.staticInit(...args2);
    var FS_init = (...args2) => FS.init(...args2);
    var FS_quit = (...args2) => FS.quit(...args2);
    var FS_findObject = (...args2) => FS.findObject(...args2);
    var FS_analyzePath = (...args2) => FS.analyzePath(...args2);
    var FS_createFile = (...args2) => FS.createFile(...args2);
    var FS_forceLoadFile = (...args2) => FS.forceLoadFile(...args2);
    var FS_createLazyFile = (...args2) => FS.createLazyFile(...args2);
    ...
    var _emscripten_glVertexPointer = (size, type, stride, ptr2) => abort("Legacy GL function (glVertexPointer) called. If you want legacy GL emulation, you need to compile with -sLEGACY_GL_EMULATION to enable legacy GL emulation.");
    ...
    var registerWebGlEventCallback = (target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) => {
      ...
      var eventHandler = { target: findEventTarget(target), eventTypeString, eventTypeId, userData, callbackfunc, handlerFunc: webGlEventHandlerFunc, useCapture };
    ...
    var _emscripten_webgl_get_uniform_d = (program, location2) => GLctx.getUniform(GL.programs[program], webglGetUniformLocation(location2));
    _emscripten_webgl_get_uniform_d.sig = "dii";
    var _emscripten_webgl_get_uniform_v = (program, location2, dst, dstLength, dstType) => writeGLArray(GLctx.getUniform(GL.programs[program], webglGetUniformLocation(location2)), dst, dstLength, dstType);
    ...
    var allocateUTF8 = (...args2) => stringToNewUTF8(...args2);
    var allocateUTF8OnStack = (...args2) => stringToUTF8OnStack(...args2);
    var demangle = (func2) => {
      ...
      return withStackSave(() => {
        try {
          var s2 = func2;
          if (s2.startsWith("__Z"))
            s2 = s2.slice(1);
          var buf = stringToUTF8OnStack(s2);
    ...
    var _emscripten_scan_registers = (func2) => {
    ...
    var _emscripten_net_send = (fd, buf, len, flags2) => Module.net?.send?.(fd, buf, len, flags2) ?? -1;
    var _emscripten_net_recv = (fd, buf, len, flags2) => Module.net?.recv?.(fd, buf, len, flags2) ?? -1;
    ...
    for (let i2 = 0; i2 < 32; ++i2)
      tempFixedLengthArray.push(new Array(i2));
    if (globalThis.setImmediate) {
    ...
    var ASM_CONSTS = { 790556: () => {
      ...
      for (const [fn, { name: name2 }] of exports)
        if (name2 === funcJsRef)
    ...
    }, 799723: () => window.innerWidth, 799753: () => window.innerHeight, 799784: ($0, $1) => {
      var buf = $0;
      var buflen = $1;
      var list = void 0;
      ...
      for (var i2 = 0; i2 < buflen; i2++) {
        setValue(buf + i2, str.charCodeAt(i2), "i8");
      }
    }, 800492: ($0) => {
...
var xash_default = Xash3D;

// public/web/cs16/engine/dist/xash3d.js
var Xash3D2 = class {
   ...
   */
  async getCVar(name2, timeoutMs = 1e3) {
    const msg = await this.waitLog(`"${name2}" is`, name2, timeoutMs);
    const searchStr = `"${name2}" is "`;
  ...
  async runEm() {
    ...
    const args2 = this.initRender(this.opts.renderer);
    const dynamicLibraries2 = [
    ...
    this.em = await xash_default({
      canvas,
      dynamicLibraries: dynamicLibraries2,
      net: this.net,
      locateFile: (path) => this.locateFile(path),
      arguments: args2,
      ...this.opts.module ?? {},
      print: (log2) => {
        this.invokeWaitLogs(log2);
        this.opts.module?.print?.(log2);
      },
      printErr: (log2) => {
        this.invokeWaitLogs(log2);
        this.opts.module?.printErr?.(log2);
      }
    });
    if (this.net) {
...
var quit_2 = (status, toThrow) => {
  throw toThrow;
};
moduleOverrides = null;
if (Module2["arguments"])
  arguments_2 = Module2["arguments"];
...
function updateMemoryViews2() {
  var b = wasmMemory2.buffer;
  Module2["HEAP8"] = HEAP82 = new Int8Array(b);
  Module2["HEAPU8"] = HEAPU82 = new Uint8Array(b);
}
var __ATPRERUN__ = [];
var __ATINIT__ = [];
var __ATPOSTRUN__ = [];
...
function preRun2() {
  ...
  callRuntimeCallbacks2(__ATPRERUN__);
}
function initRuntime2() {
  runtimeInitialized2 = true;
  callRuntimeCallbacks2(__ATINIT__);
}
function postRun2() {
  if (Module2["postRun"]) {
  ...
  callRuntimeCallbacks2(__ATPOSTRUN__);
}
...
function addRunDependency2(id) {
  var _a;
  runDependencies2++;
  (_a = Module2["monitorRunDependencies"]) === null || _a === void 0 ? void 0 : _a.call(Module2, runDependencies2);
}
...
function init2(filePathOrBuf) {
  var info2 = getWasmImports2();
  function receiveInstance(instance2, module2) {
    wasmExports2 = instance2.exports;
    wasmMemory2 = wasmExports2["f"];
    updateMemoryViews2();
    addOnInit2(wasmExports2["g"]);
  ...
  function instantiateArrayBuffer2(receiver) {
    return getBinaryPromise(filePathOrBuf).then(function(binary2) {
      var result = WebAssembly.instantiate(binary2, info2);
      return result;
    ...
    if (filePathOrBuf && filePathOrBuf.byteLength > 0) {
      ...
      return fetch(filePathOrBuf, { credentials: "same-origin" }).then(function(response) {
        ...
        return result.then(receiveInstantiationResult, function(reason) {
          ...
          return instantiateArrayBuffer2(receiveInstantiationResult);
        });
      });
    } else {
      return instantiateArrayBuffer2(receiveInstantiationResult);
  ...
  if (Module2["instantiateWasm"]) {
    try {
      var exports = Module2["instantiateWasm"](info2, receiveInstance);
      return exports;
    } catch (e) {
...
var __abort_js2 = () => abort2("");
var runtimeKeepaliveCounter2 = 0;
var __emscripten_runtime_keepalive_clear2 = () => {
  noExitRuntime2 = false;
  runtimeKeepaliveCounter2 = 0;
};
var timers2 = {};
var handleException2 = (e) => {
  if (e instanceof ExitStatus2 || e == "unwind") {
    return EXITSTATUS2;
  }
  quit_2(1, e);
};
var keepRuntimeAlive2 = () => noExitRuntime2 || runtimeKeepaliveCounter2 > 0;
var _proc_exit2 = (code) => {
  var _a;
  EXITSTATUS2 = code;
  ...
  quit_2(code, new ExitStatus2(code));
};
...
var _emscripten_get_now2 = () => performance.now();
var __setitimer_js2 = (which, timeout_ms) => {
  if (timers2[which]) {
    clearTimeout(timers2[which].id);
    delete timers2[which];
  }
  if (!timeout_ms)
    return 0;
  var id = setTimeout(() => {
    delete timers2[which];
    callUserCallback2(() => __emscripten_timeout2(which, _emscripten_get_now2()));
  }, timeout_ms);
  timers2[which] = { id, timeout_ms };
  return 0;
};
...
var _emscripten_resize_heap2 = (requestedSize) => {
  ...
  for (var cutDown = 1; cutDown <= 4; cutDown *= 2) {
    ...
    var newSize = Math.min(maxHeapSize, alignMemory2(Math.max(requestedSize, overGrownHeapSize), 65536));
...
var wasmImports2 = {
  c: __abort_js2,
  b: __emscripten_runtime_keepalive_clear2,
  d: __setitimer_js2,
  e: _emscripten_resize_heap2,
  a: _proc_exit2
};
var wasmExports2;
var _ZSTD_isError = Module2["_ZSTD_isError"] = (a0) => (_ZSTD_isError = Module2["_ZSTD_isError"] = wasmExports2["h"])(a0);
var _ZSTD_compressBound = Module2["_ZSTD_compressBound"] = (a0) => (_ZSTD_compressBound = Module2["_ZSTD_compressBound"] = wasmExports2["i"])(a0);
var _ZSTD_createCCtx = Module2["_ZSTD_createCCtx"] = () => (_ZSTD_createCCtx = Module2["_ZSTD_createCCtx"] = wasmExports2["j"])();
var _ZSTD_freeCCtx = Module2["_ZSTD_freeCCtx"] = (a0) => (_ZSTD_freeCCtx = Module2["_ZSTD_freeCCtx"] = wasmExports2["k"])(a0);
var _ZSTD_compress_usingDict = Module2["_ZSTD_compress_usingDict"] = (a0, a1, a2, a3, a4, a5, a6, a7) => (_ZSTD_compress_usingDict = Module2["_ZSTD_compress_usingDict"] = wasmExports2["l"])(a0, a1, a2, a3, a4, a5, a6, a7);
var _ZSTD_compress = Module2["_ZSTD_compress"] = (a0, a1, a2, a3, a4) => (_ZSTD_compress = Module2["_ZSTD_compress"] = wasmExports2["m"])(a0, a1, a2, a3, a4);
var _ZSTD_createDCtx = Module2["_ZSTD_createDCtx"] = () => (_ZSTD_createDCtx = Module2["_ZSTD_createDCtx"] = wasmExports2["n"])();
var _ZSTD_freeDCtx = Module2["_ZSTD_freeDCtx"] = (a0) => (_ZSTD_freeDCtx = Module2["_ZSTD_freeDCtx"] = wasmExports2["o"])(a0);
var _ZSTD_getFrameContentSize = Module2["_ZSTD_getFrameContentSize"] = (a0, a1) => (_ZSTD_getFrameContentSize = Module2["_ZSTD_getFrameContentSize"] = wasmExports2["p"])(a0, a1);
var _ZSTD_decompress_usingDict = Module2["_ZSTD_decompress_usingDict"] = (a0, a1, a2, a3, a4, a5, a6) => (_ZSTD_decompress_usingDict = Module2["_ZSTD_decompress_usingDict"] = wasmExports2["q"])(a0, a1, a2, a3, a4, a5, a6);
var _ZSTD_decompress = Module2["_ZSTD_decompress"] = (a0, a1, a2, a3) => (_ZSTD_decompress = Module2["_ZSTD_decompress"] = wasmExports2["r"])(a0, a1, a2, a3);
var _malloc2 = Module2["_malloc"] = (a0) => (_malloc2 = Module2["_malloc"] = wasmExports2["s"])(a0);
var _free2 = Module2["_free"] = (a0) => (_free2 = Module2["_free"] = wasmExports2["t"])(a0);
var __emscripten_timeout2 = (a0, a1) => (__emscripten_timeout2 = wasmExports2["v"])(a0, a1);
var calledRun;
dependenciesFulfilled2 = function runCaller() {
  if (!calledRun)
    run2();
  if (!calledRun)
    dependenciesFulfilled2 = runCaller;
};
function run2() {
  if (runDependencies2 > 0) {
    return;
...
var init3 = (path) => __awaiter2(void 0, void 0, void 0, function* () {
  const url = new URL(`./zstd.wasm`, import.meta.url).href;
  Module2["init"](path !== null && path !== void 0 ? path : url);
  yield waitInitialized();
});

// public/web/cs16/tar-stream.js
var BLOCK = 512;
var MAX_FILE_BYTES = 64 * 1024 * 1024;
var MAX_TOTAL_BYTES = 2 * 1024 * 1024 * 1024;
var decoder = new TextDecoder();
function fieldText(block, start3, length) {
  const end = block.subarray(start3, start3 + length).indexOf(0);
  return decoder.decode(block.subarray(start3, start3 + (end < 0 ? length : end))).trim();
}
function octal(block, start3, length, label) {
  const raw = fieldText(block, start3, length);
  if (!raw) return 0;
...
var StreamCursor = class {
  ...
  async take(size) {
    ...
    while (written < size) {
      ...
      out2.set(head.subarray(0, count), written);
      written += count;
      this.queued -= count;
...
function verifyChecksum(block) {
  ...
  let actual = 0;
  for (let i2 = 0; i2 < block.length; i2++) actual += i2 >= 148 && i2 < 156 ? 32 : block[i2];
  if (actual !== expected) throw new Error(`TAR \u5934\u6821\u9A8C\u5931\u8D25\uFF08\u671F\u671B ${expected}\uFF0C\u5B9E\u9645 ${actual}\uFF09`);
}
async function unpackTarStream(stream, onFile, onProgress) {
  ...
  for (; ; ) {
    ...
    const name2 = fieldText(header, 0, 100);
...
function mark(name2, extra) {
  const t = ((performance.now() - t0) / 1e3).toFixed(2);
  if (!(name2 in marks)) marks[name2] = Number(t);
  log(`[${t}s] ${name2}${extra ? " \u2014 " + extra : ""}`);
  setPhase(name2);
}
...
function guardStream(stream, label, url) {
  ...
  const key2 = new URL(url, location.href).pathname;
  return new ReadableStream({
    async pull(controller) {
      ...
      try {
        ...
        window.__probe.net.byUrl[key2] = (window.__probe.net.byUrl[key2] || 0) + next.value.byteLength;
        setPhase(phase);
...
var ZSTD_FORMAT = "8bitgo.cs16.zstd-chunks.v1";
var SHA256_RE = /^[a-f0-9]{64}$/;
var chunkPromises = /* @__PURE__ */ new Map();
var catalogPromise;
var wasmZstdPromise;
var nativeZstd = (() => {
  try {
...
async function ensureWasmZstd() {
  if (!wasmZstdPromise) wasmZstdPromise = init3(`${ASSET_ROOT}/zstd.wasm?v=${ASSET_VERSION}`);
...
async function fetchChunk(chunk, label) {
  const key2 = chunk.compressedSha256;
  if (!chunkPromises.has(key2)) {
    chunkPromises.set(key2, (async () => {
      let lastError;
  ...
  try {
    return await chunkPromises.get(key2);
  } catch (error) {
    chunkPromises.delete(key2);
    throw error;
  }
}
function zstdPackStream(pack, key2) {
  let next = 0;
...
function isGzip(b) {
  return b.length >= 2 && b[0] === 31 && b[1] === 139;
}
var LIB_FILES = {
...
async function placeLibs(xash) {
  ...
  for (const [name2, buf] of files) {
    for (const dir of ["/", BASE]) {
      const full = dir + name2;
...
function ensureBinds(xash) {
  ...
  for (const dir of ["", "cstrike/", "valve/"]) {
    try {
      FS2.writeFile(BASE + dir + "autoexec.cfg", buf);
    } catch {
    }
  }
...
async function start2() {
  ...
  const xash = new Xash3D2({
    module: {
      ...
      printErr: (s2) => {
        window.__probe.log.push("[err] " + s2);
        log("[err] " + s2, "err");
      },
      locateFile: locateFile2
    },
    canvas: canvasEl,
    libraries: LIBS_MAP
  });
...
if (Q.has("map")) {
  start2().catch(showError);
} else {
  $("start")?.addEventListener("click", () => start2().catch(showError));
}
