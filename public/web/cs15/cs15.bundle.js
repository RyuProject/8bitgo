// public/web/cs15/engine/dist/constants.js
...
// public/web/cs15/engine/dist/generated/xash.js
var Xash3D = (() => {
...
`;
    ...
    function assignWasmExports(wasmExports2) {
      _Cmd_ExecuteString = Module["_Cmd_ExecuteString"] = wasmExports2["Cmd_ExecuteString"];
      _malloc = wasmExports2["malloc"];
      _free = wasmExports2["free"];
      _strerror = wasmExports2["strerror"];
      _main = Module["_main"] = wasmExports2["__main_argc_argv"];
      _htons = wasmExports2["htons"];
      _htonl = wasmExports2["htonl"];
      _ntohs = wasmExports2["ntohs"];
      _fileno = wasmExports2["fileno"];
      _calloc = wasmExports2["calloc"];
      _realloc = wasmExports2["realloc"];
      _getErrnoLocation = Module["_getErrnoLocation"] = wasmExports2["getErrnoLocation"];
      ...
      ___cxa_demangle = wasmExports2["__cxa_demangle"];
      ...
      ___stack_pointer = wasmExports2["__stack_pointer"];
...
// public/web/cs15/engine/dist/xash3d.js
var Xash3D2 = class {
...
function safePackPath(value) {
  const name2 = String(value).replaceAll("\\", "/");
  if (!name2 || name2.startsWith("/") || name2.includes("\0")) return null;
  const parts = name2.split("/");
  if (parts.some((part) => !part || part === "." || part === ".." || /[\x00-\x1f:]/.test(part))) return null;
  return name2;
}
async function unpackStoredZipStream(stream, sink, expected = {}) {
  const packName = expected.name || "\u8D44\u6E90\u5305";
  const reader = stream.getReader();
  const decoder = new TextDecoder();
  const byteLimit = Number.isSafeInteger(expected.bytes) ? expected.bytes : Number.isSafeInteger(expected.maxBytes) ? expected.maxBytes : 1280 * 1024 * 1024;
  ...
  async function readExact(length) {
    const out2 = new Uint8Array(length);
    let written = 0;
    while (written < length) {
      ...
      out2.set(chunk.subarray(offset, offset + take), written);
      offset += take;
      written += take;
    }
    return out2;
  ...
  async function copyFile(name2, length, expectedCrc) {
    const target = await sink.open(name2, length);
    let remaining = length;
    let crc = 4294967295;
    try {
      while (remaining > 0) {
        if (!await refill()) throw new Error(`${packName} \u7684 ${name2} \u63D0\u524D\u7ED3\u675F`);
        const take = Math.min(remaining, chunk.length - offset);
    ...
    if (actualCrc !== expectedCrc) {
      throw new Error(`${packName} \u7684 ${name2} CRC-32 \u4E0D\u7B26\uFF08\u4F20\u8F93\u6216 R2 \u5BF9\u8C61\u5DF2\u635F\u574F\uFF09`);
    }
  }
  try {
    while (true) {
      ...
      const flags2 = view.getUint16(2, true);
      ...
      if (flags2 & 1) throw new Error(`${packName} \u542B\u52A0\u5BC6\u6761\u76EE\uFF1A${rawName}`);
      if (flags2 & 8) throw new Error(`${packName} \u4E0D\u652F\u6301 data descriptor\uFF1A${rawName}`);
      if (method !== 0 || compressedSize !== rawSize) {
        throw new Error(`${packName} \u5FC5\u987B\u662F store ZIP\uFF1A${rawName}`);
      }
      const directory = rawName.endsWith("/");
      const name2 = safePackPath(directory ? rawName.slice(0, -1) : rawName);
      if (!name2) throw new Error(`${packName} \u542B\u4E0D\u5B89\u5168\u8DEF\u5F84\uFF1A${rawName}`);
      if (rawSize > fileLimit) throw new Error(`${packName} \u7684 ${name2} \u5355\u6587\u4EF6\u5F02\u5E38\u5927`);
      if (bytes + rawSize > byteLimit) throw new Error(`${packName} \u5C55\u5F00\u5927\u5C0F\u8D85\u8FC7\u6E05\u5355\u4E0A\u9650`);
      if (directory) {
        await sink.mkdir?.(name2);
        await skip(compressedSize);
        continue;
      }
      await copyFile(name2, compressedSize, expectedCrc);
      files++;
      bytes += rawSize;
    }
  } finally {
...
// public/web/cs15/cs15.js
var BASE = "/rodir/";
var ASSET_VERSION = "20260923-packs2";
var Q = new URLSearchParams(location.search);
var VGUI_MENUS = Q.get("vgui") || "0";
var BOTS_ENABLED = Q.get("bots") !== "0";
var PAGE_ROOT = new URL("./", document.baseURI);
var ASSET_ROOT = new URL(Q.get("root") || "./", PAGE_ROOT).href.replace(/\/$/, "");
var LOCAL_PACK_HOSTS = /* @__PURE__ */ new Set(["localhost", "127.0.0.1", "::1"]);
var DEFAULT_PACKS_ROOT = LOCAL_PACK_HOSTS.has(location.hostname) ? ASSET_ROOT + "/packs" : "https://assets.8bitgo.com/web/cs15/packs";
var PACKS_ROOT = (Q.get("packsroot") ? new URL(Q.get("packsroot"), PAGE_ROOT).href : DEFAULT_PACKS_ROOT).replace(/\/$/, "");
var MAP_OPTIONS = {
  cs: [
    ["de_dust2", "de_dust2"],
    ["de_dust", "de_dust"],
...
function setPhase(text) {
  ...
  const n = $("net");
  if (n) n.textContent = `\u5DF2\u4E0B\u8F7D ${(window.__probe.net.total / 1048576).toFixed(1)} MB`;
  const bar = document.querySelector("#bar > i");
  if (bar) bar.style.width = Math.min(100, window.__probe.net.total / 1048576 / 100 * 100).toFixed(1) + "%";
}
var missingNavWarned = false;
function engineOutput(message, isError = false) {
  ...
  if (/Failed to load .*\.nav.*navigation map/i.test(text) || /Navigation file not found/i.test(text)) {
    if (!missingNavWarned) {
      ...
      const warning = BOTS_ENABLED ? "[bots] \u9996\u6B21\u8FD0\u884C\u6B63\u5728\u751F\u6210\u673A\u5668\u4EBA\u5BFC\u822A\uFF0C\u901A\u5E38\u7EA6 20 \u79D2\uFF1B\u5B8C\u6210\u540E\u4F1A\u81EA\u52A8\u52A0\u5165 Bot\u3002" : "[bots] \u5F53\u524D\u5730\u56FE\u6CA1\u6709\u673A\u5668\u4EBA\u5BFC\u822A\uFF0C\u673A\u5668\u4EBA\u5DF2\u505C\u7528\uFF1B\u6B63\u5E38\u6E38\u620F\u4E0D\u53D7\u5F71\u54CD\u3002";
...
async function placeLibs(xash, libFiles) {
  for (const [name2, url] of Object.entries(libFiles)) {
    const res = await fetch(url + (url.includes("?") ? "&" : "?") + "v=" + ASSET_VERSION);
    if (!res.ok) {
      log(`\u26A0\uFE0F \u53D6\u4E0D\u5230 ${name2}\uFF08HTTP ${res.status}\uFF09`, "err");
      continue;
    }
  ...
  if (index?.schema === 2 && index.profiles?.[game] && index.packs) {
    ...
    return keys.map((key) => {
      ...
      return {
        ...
        file,
        fallback,
        files: Number.isSafeInteger(item.files) ? item.files : void 0,
        bytes: Number.isSafeInteger(item.bytes) ? item.bytes : void 0
      };
    });
  }
...
async function loadPack(xash, pack) {
  ...
  return unpackStoredZipStream(stream, {
    mkdir(name2) {
      FS2.mkdirTree(BASE + name2);
    },
    open(name2) {
      const full = BASE + name2;
      const cut = full.lastIndexOf("/");
      if (cut > 0) FS2.mkdirTree(full.slice(0, cut));
      const target = FS2.open(full, "w");
      return {
        write(part) {
          const written = FS2.write(target, part, 0, part.length);
          if (written !== part.length) throw new Error(`${name2} \u53EA\u5199\u5165 ${written}/${part.length} \u5B57\u8282`);
        },
        close() {
          FS2.close(target);
        }
...
function compatPatch(xash) {
  const FS2 = xash.em.FS;
  FS2.mkdirTree(BASE + "cstrike/sprites");
  for (const [f, corner] of [
    ...
    FS2.writeFile(`${BASE}cstrike/sprites/${f}.tga`, makeScopeTga(corner));
  }
  FS2.mkdirTree(BASE + "cstrike/resource");
  for (const [f, text] of Object.entries(COMPAT_RESOURCE)) {
    const p = `${BASE}cstrike/resource/${f}`;
    if (!FS2.analyzePath(p).exists) FS2.writeFile(p, new TextEncoder().encode(text));
  }
...
async function loadHudFont(xash) {
  ...
  try {
    ...
    if (!res.ok) {
      log("\u26A0\uFE0F \u53D6\u4E0D\u5230 HUD \u5B57\u4F53\uFF08HTTP " + res.status + "\uFF09", "err");
      return;
    }
    const buf = new Uint8Array(await res.arrayBuffer());
...
var BOT_NAV_DB = "8bitgo-cs15-bot-nav";
var BOT_NAV_STORE = "maps";
var activeLaunch = null;
var automaticBotRequested = false;
...
function watchGeneratedBotNav(xash, map) {
  const path = `${BASE}cstrike/maps/${map}.nav`;
  const deadline = performance.now() + 12e4;
  const timer = setInterval(async () => {
    ...
    if (found.exists && xash.em.FS.stat(path).size >= 1024) {
      ...
      botLog(saved ? `[bots] ${map}.nav \u5DF2\u751F\u6210\u5E76\u7F13\u5B58\uFF1B\u4EE5\u540E\u8FDB\u5165\u8BE5\u5730\u56FE\u4F1A\u76F4\u63A5\u52A0\u8F7D\u3002` : `[bots] ${map}.nav \u5DF2\u751F\u6210\uFF0C\u4F46\u6D4F\u89C8\u5668\u62D2\u7EDD\u6301\u4E45\u7F13\u5B58\uFF1B\u672C\u5C40\u4ECD\u53EF\u6B63\u5E38\u4F7F\u7528\u3002`);
      return;
    }
...
function syncMapOptions(preferredMap) {
  const gameSelect2 = $("game");
  const mapSelect = $("map");
  if (!gameSelect2 || !mapSelect) return;
  const options = MAP_OPTIONS[gameSelect2.value] || MAP_OPTIONS.cs;
...
async function start2() {
  ...
  const xash = new Xash3D2({
    module: {
      arguments: args2,
      print: (s2) => engineOutput(s2),
      printErr: (s2) => engineOutput(s2, true),
...
function showError(e) {
  ...
  mark("\u5931\u8D25");
  const box = $("errbox");
  if (box) box.textContent = "\u274C " + (e && e.message ? e.message : msg) + "\n\u8BF7\u5237\u65B0\u9875\u9762\u540E\u91CD\u8BD5\u3002";
...
if (Q.has("game") || Q.has("map")) {
  ...
  $("start")?.addEventListener("click", () => start2().catch(showError), { once: true });
}
