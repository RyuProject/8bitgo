...
最后核对：2026-09-23。当前使用稳定版 `@ruffle-rs/ruffle@0.6.0`，运行文件位于
...
- `quality: low`：关闭高成本的高阶抗锯齿；
- iframe 内 `devicePixelRatio ≤ 1`：不改变页面和游戏时间轴，只降低 Ruffle 画布像素数；
...
DPR 2 的屏幕原本会创建宽高各两倍的画布，也就是四倍像素量。钳到 1 后，填充量、画布显存和
直播采集源分辨率都会同步下降。代价是高分屏边缘不如原先锐利，这正是固定「流畅优先」的取舍。
...
`quality: low` 只影响舞台抗锯齿，不能阻止 Ruffle 按 DPR 建大画布。新的 iframe 级 DPR 上限使 Retina
屏从 4× 像素量回到 1×；覆盖失败时安全保留浏览器原值，不影响启动。
...
- Retina 设备上 Ruffle canvas 像素尺寸不再乘以设备 DPR；
