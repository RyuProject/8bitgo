...
在开放平台这一侧多加一道令牌，挡不住任何人（爬网页就有），
却让「我只想在自己的小设备上列个游戏菜单」这件事先要注册应用、存 key、写取令牌的重试逻辑。
门要留给**真的有代价的东西**：ROM 的字节、用户的存档、别人的收藏。
...
公开接口的规矩是：**没有 `Authorization` 头 = 匿名放行；有头但验不过 = `401`。**
不能写成「验不过就当匿名」。后者看起来更宽容，实际是把接入方的令牌过期
变成了一个**没有症状的故障**：列表照常 200，直到某天调 ROM 才报错，
而那句错误指向的是 ROM 权限，不是「你的令牌两小时前就过期了」。
（同一个形状的 bug 在站内后台口令那边真的发生过：一枚错的 admin token
遮住了正确的登录态，报出来的是「权限不足：需要 content:edit」。）

反过来，带着**有效**令牌调公开接口时**不校验 scope** ——
否则就成了「匿名 curl 读得到，带着只有 `games.rom` 的令牌反而 403」。
...
> 只在出口处筛掉的话，`total` 会把它算进去而条目里没有，
> 症状是「total 说 100，一页页翻到底只有 87 款」，且夹着它的那页比 `page_size` 短。
> 接入方查不出原因，只会觉得这个接口时好时坏。
