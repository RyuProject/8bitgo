...
async function loadHome() {
  ...
  /**
   * curated 要一路带到前台：前台靠它决定「站长精选」那一栏画不画
   * （PickedSection，一款都没钦点就整栏不出现）。
   *
   * 没钦点时 popular 退回按游玩次数自动排 —— 那份数据这时只喂首页横幅，
   * 精选那一栏不会拿它冒充「站长挑的」。
   */
  ...
  return {
    ...
    /**
     * 首页「最多人玩的模拟器游戏」那一栏（前台 MostPlayedSection）。
     * **始终**是按游玩次数排出来的那份真榜，不受精选影响。
     *
     * 为什么不让前台直接用上面的 popular：钦点了首页排序之后 popular 装的是人排的顺序，
     * 拿它当榜就是让界面陈述一件不成立的事。这个字段和它是两回事，别合并。
     *
     * 切的是同一次查询的结果，**不多跑一次 SQL**。
     *
     * ⚠️ 字段名保持 hottest 不动 —— 线上前端按这个名字取数，改名等于让所有还没
     * 更新的前端整栏消失（2026-09-12 前后端版本错位那次就是这个症状）。
     */
