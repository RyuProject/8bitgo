...
--    整个脚本会中途报错。
--
-- ⚠️⚠️ 还有一件上面那份名单里看不出来的事：**所有玩家的云存档会全部失效。**
--    `saves` 表不在删除名单里，所以它活下来了 —— 但里面每一行的 user_id 指向的用户
--    已经被删掉了。用户 id 是随机生成的（'u_' + 6 字节随机），重新注册拿到的是新 id，
--    旧存档**永远回不来**，只是占着表躺在库里。
--    执行前先看一眼有多少：  SELECT COUNT(*) FROM saves;
--    要保的话，得自己先把 users(id, email) 和 saves 一起导出来，重建之后按 email 认领。
--
-- 执行前先跑这一句，确认你要丢掉什么：
--
