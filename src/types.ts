...
export interface Game {
   ...
   * 只要有任意一款设了，首页第一栏就只出这些，标题也会从「最多人玩的模拟器游戏」
   * 换成「站长精选」，并摘掉 #1 #2 的排名角标 —— 手挑的顺序不能顶着「按游玩次数排序」的说法。
   *
   * ⚠️ 那份按游玩次数排的真榜**不会消失**：它在首页的「最热门的游戏」那一栏
   * （HottestSection，见 server/src/content.js 的 hottest）。这两个名字别搞混 ——
   * 这段说明以前写的是「标题变成『最热门的游戏』」，那是错的，而且正好撞上了
   * 另一栏真实存在的名字，看的人会以为第一栏变成了那一栏。
   */
  homeRank?: number
