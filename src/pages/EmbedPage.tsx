    ...
    body = (
      ...
      >
        <EmulatorPlayer
          ...
          maxPlayers={1}
          /*
            这一页把整块视口都给了播放器（下面那个 min-h-0 flex-1），所以让它按游玩布局排：
            画面吃满高度、引擎自带的按键叠在下面。不这么给的话播放器会套一个自己的比例框，
            而这一页的框比例是**贴代码的人选的**（分享面板给的三个预设都是 4:3），
            再塞一个 16:9 进去就是 letterbox 两次 —— 640×480 的框里画面只剩 480×360。

            ⚠️ `h-full` **原来写的是 `max-sm:h-full`，2026-09-08 去掉了断点**。
            它要和 EmulatorPlayer 里的 `embedFill` 严格对齐，而那边原本也要求 `narrow`
            （`max-width: 639.98px`）—— 三个预设 640 / 800 / 960 一个都不命中，
            于是这条高度链在真实嵌入里从来没接上过。详见 embedFill 那段注释里的实测表。
            `fill` 是开关，缺了它这个 h-full 不生效。
          */
          fill
          className="h-full"
          icon={game.icon}
