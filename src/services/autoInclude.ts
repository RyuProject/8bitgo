 ...
 *  2. **带查询串的地址一条都不推。** `/games?platform=gba`、`/games?q=…` 这类
 *     没有独立收录价值，只允许爬虫在已经知道 URL 时重抓以读取 noindex / canonical；而分页页面
 ...
 *     → 守卫放行 → 推出去的却是那个非收录目标 `?q=`。2026-09-07 查出来的，
 *     GSC 里那批搜索页就是这么被主动送出去的。
