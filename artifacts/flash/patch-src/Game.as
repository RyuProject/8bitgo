package
{
   ...
   public class Game extends MovieClip
   {
      ...
      // FFDec 重编译时不能保留“const 声明后在构造器赋值”的原始字节码形态，
      // 否则 Ruffle 会报 #1074 并在读取内嵌关卡 JSON 前终止构造。
      private var JsonData:Class = Game_JsonData;
      ...
      {
         // 原发行商的 Configar / Spil SDK 已经下线；继续 connect 会让开场黑屏等到网络超时。
         // 游戏所需文案本来就嵌在 LocalizationPack 里，由 BrandingManager 的离线回退读取。
      ...
      public function GameReady() : *
      {
         ...
         // MochiBot 的 core.swf 服务早已失效；它只做统计，却会制造一次跨域失败和额外等待。
         this.gotoAndStop(2);
         this.Muted = false;
