...
2. 房主掉线时房间**不立刻解散**，进入「等待新房主」状态保留 **60 秒**，
   按加入顺序选出最早进来的那位访客
3. 被选中的人自动取存档 → 载入自己的模拟器 → 重新开房 → 其他人跟过去。
   **原来的邀请链接继续有效**（服务器把旧房间 id 做成别名）
4. 60 秒内没人接手（比如大家都走了）才真的解散，提示「房主已离开」
...
NETPLAY_HOST_GRACE_MS=400 node scripts/test-netplay.mjs
```

27 项，覆盖开房 / 加入 / 信令定向转发 / 满员 / 密码 / 存档托管 / 掉线选新房主 /
迁移后老链接仍有效 / 超时解散 / 跨房间信令隔离。改了 `src/netplay.js` 就跑一遍。
