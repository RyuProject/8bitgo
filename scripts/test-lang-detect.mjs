...
check('seo.ts 的 x-default 优先指向 FALLBACK_LANG（没有英文正文时跟 canonical）', () => {
  /*
    ...
    按需翻译详情页可能根本没有英文正文。这时 x-default 必须跟它真正可用的 canonical，
    不能为了凑英语而指向一份回退页面。普通页面仍然优先 FALLBACK_LANG。
  */
  const seo = read('src/services/seo.ts')
  assert.match(seo, /available\.includes\(FALLBACK_LANG\) \? FALLBACK_LANG : canonicalLanguage/)
  assert.match(seo, /\['x-default', urlInLang\(languagePlan\.defaultLanguage\)\]/)
  assert.match(seo, /HREFLANG\[code\], urlInLang\(code\)/)
})
